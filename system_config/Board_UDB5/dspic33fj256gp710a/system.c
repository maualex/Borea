...
_FOSC(POSCMD_XT & OSCIOFNC_ON & FCKSM_CSDCMD) //funzione che scrive sul registro FOSC
_FWDT(FWDTEN_OFF)
_FPOR(FPWRT_PWR128)
_FICD(ICS_PGD2 & JTAGEN_OFF)

unsigned char flag_actuator, flag_control_init, flag_calibration, flag_control, flag_calibration_request;
...
void SYSTEM_Initialize (void)
{
    ...
    //OSCTUN              = 0;    // oscillatore FRC
    //RCONbits.SWDTEN     = 0;    // Disable Watch Dog Timer
    while(OSCCONbits.LOCK!=1) {};   // wait PLL
    
	TRISE 						= 0x0000;		// configuro tutti i pin della porta E come uscita (led pin)
	LATE  						= ALL_LED_OFF;  //all led turned off
	LED_RED 					= LED_ON;
...
//UART1 configuration (GPS module)	
	U1BRG = 0; //BAUD_RATE_SETTING;		// 1) BAUD Rate Setting for 4800, and Fcy = 40MHz
...
//UART2 configuration (XBee module)
    ...
    //U2STAbits.URXISEL   = 0b11;    //interrupt is generated when 4 bytes has been received
    U2STAbits.URXISEL   = 0;    //interrompe quando riceve un byte 
    ...
    U2BRG               = floor((SYS_CLK_FrequencySystemGet()/BAUD_RATE_SETTING)/16) - 1; //64; 
    
    //interrupt controller configuration
    IEC1bits.U2RXIE     = 1;    //interrupt on recieve data enabled
    IEC1bits.U2TXIE     = 1;    //interrupt on trasmit data disabled
    
    IPC7bits.U2RXIP     = 6;    //priority uart reception greater than timer 
    IPC7bits.U2TXIP     = 6;
    //clear flags
    IFS1bits.U2RXIF     = 0;
    IFS1bits.U2TXIF     = 0;
...
// CONFIGURAZIONE ADC2  (BATTERY VOLTAGE) --> si potrebbe gestire con interrupt ad una frequenza lenta (5Hz?)
    AD2CON1bits.ADON = 0;     //spengo il modulo ADC2 per fare la configurazione

    
    //analog pin for sonar connection
...
//    TRISAbits.TRISA14   = 0;
    ...
    //i pin che seguono non sono disponibili
...
{
    //LATA14 = a;   //nella UDB4
    LATAbits.LATA5 = a;      //nella UDB5
}

inline bool USER_SdSpiGetCd_1(void)
{
    return 1; //(!PORTFbits.RF0) ? true : false;
}

inline bool USER_SdSpiGetWp_1(void)
{
    return 0; //(PORTFbits.RF1) ? true : false;
}
