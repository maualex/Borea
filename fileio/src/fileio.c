...
FILEIO_ERROR_TYPE FILEIO_DriveMount (char driveId, const FILEIO_DRIVE_CONFIG * driveConfig, void * mediaParameters)
{
    ...
    drive = FILEIO_CharToDrive (driveId); //la prima volta torna NULL perchè dopo l'inizializzazione nessun drive ha una lettera assegnata o è montato

    if (drive == NULL)      //il drive slot è aperto (cioè non c'è montato nessun drive ancora) e non ha ancora una lettera
    {
        for (i = 0; i < FILEIO_CONFIG_MAX_DRIVES; i++)
        {
            if (gDriveSlotOpen[i])
            {
                gDriveSlotOpen[i] = false;                  //ora il drive non è più libero 
                gDriveArray[i].driveId = driveId;           //assegna la lettera all'unità
                gDriveArray[i].driveConfig = driveConfig;   //passo le funzioni del driver che verranno utilizzate dalla libreria IO su questo drive
                drive = &gDriveArray[i];
                break;
            }
        }
